...
function Main({ blogs }) {
  ...
  return (
    <div className="md:grid lg:grid sm:grid-cols-1  md:grid-cols-3 lg:grid-cols-3 p-4 md:p-2 lg:p-2 gap-5 mb-9 max-w-6xl m-auto md:mt-7 lg:mt-7 text-white">
      {/* Left Div */}
      <div className="col-span-2">
        <Link to={"/blog/" + blogs[0]?._id}>
          <img
            className=" rounded-2xl object-contain h-80 md:h-fit lg:h-fit"
            src={`${BASE_URL}/uploads/` + blogs[0]?.photos?.[0]}
            alt={blogs[0]?.title}
          />
        </Link>
        <Link to={"/blog/" + blogs[0]?._id}>
        ...
        <div className="flex items-center mt-2 truncate">
          <img
            ...
            src={`${BASE_URL}/userphotos/` + blogs[0]?.owner?.photos[0]}
        ...
        <div className="col-span-1">
          <Link to={"/blog/" + blogs[1]?._id}>
            <img
              ...
              src={`${BASE_URL}/uploads/` + blogs[1]?.photos?.[0]}
              alt={blogs[1]?.title}
            />
          </Link>
          <Link to={"/blog/" + blogs[1]?._id}>
          ...
          <div className="flex items-center mt-2 truncate">
            <img
              className="w-10 h-10 rounded-full mr-2"
              src={`${BASE_URL}/userphotos/` + blogs[1]?.owner?.photos[0]}
              alt="Blogger Avatar"
            />
            <div className="flex items-center gap-3 text-center">
        ...
        <div className="col-span-1">
          <Link to={"/blog/" + blogs[2]?._id}>
            <img
              ...
              src={`${BASE_URL}/uploads/` + blogs[2]?.photos?.[0]}
              alt={blogs[2]?.title}
            />
          </Link>
          <Link to={"/blog/" + blogs[1]?._id}>
          ...
          <div className="flex items-center mt-2 truncate">
            <img
              ...
              src={`${BASE_URL}/userphotos/` + blogs[2]?.owner?.photos[0]}
              alt="Blogger Avatar"
            />
            <div className="flex items-center gap-3 text-center">
              <p className="font-medium text-gray-300">
